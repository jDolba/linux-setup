...
`key-keeper-unlock KEY-NAME [MINUTES]` deliberately grants the invoking user's existing SSH agent temporary use of one identity (five minutes by default). During that window every process able to use that agent, including AI agents with shell access, can authenticate as that identity. Keep unlock periods short; private-key bytes must never be written to user-controlled output or files.
